...
def load_checkpoint(module_name: str, path: str, step: int, device: str = "cpu") -> dict:
    ...
    Args:
        ...
        device (str): String specifying how to remap storage locations (default = "device_idcpu").
...
def get_model_device(model):
    """
    Returns the device of the first parameter or buffer in the model.
    Supports plain nn.Module and wrapped models like PeftModel.
    """
    try:
        # For PEFT models, unwrap to base model if possible
        if hasattr(model, "base_model") and hasattr(model.base_model, "model"):
            model = model.base_model.model
    except Exception:
        pass  # fallback to original model if unwrapping fails

    # Check parameters
    for param in model.parameters():
        return param.device

    # Check buffers if no parameters
    for buffer in model.buffers():
        return buffer.device

    return "unknown (no parameters or buffers)"
...
def wrap_ddp(module: nn.Module, device_id: int, find_unused: bool = False) -> DDP:
    ...
    """
    print('++++ device: ',get_model_device(module))
    return module # remove torchrun
...
def run_diffusion_sampling(
    ...
    for t in action_head.noise_scheduler.timesteps:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat1632323232323232323232323232323232323232323232323232323232323232):
            output = vla(
                input_ids=batch["input_ids"].to(device_id),
                attention_mask=batch["attention_mask"].to(device_id),
                pixel_values=batch["pixel_values"].to(torch.bfloat16).to(device_id),
...
def check_nested_tensor_devices(obj, prefix=''):
    import torch
    if isinstance(obj, torch.Tensor):
        print(f"{prefix}Tensor: {obj.device}")
    elif isinstance(obj, dict):
        for k, v in obj.items():
            check_nested_tensor_devices(v, f"{prefix}{k}.")
    elif isinstance(obj, (list, tuple)):
        for i, v in enumerate(obj):
            check_nested_tensor_devices(v, f"{prefix}[{i}].")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_proprio:
        proprio_projector = init_module(
            ...
            cfg,
            device_id,
            {"llm_dim": vla.llm_dim, "proprio_dim": PROPRIO_DIM}, # remove torchrun
        )
